...
export function TagButtonOutline({ name, handleTag }: TagButton) {
    return (
        <div className="text-gray-700 border border-gray-700 font-medium rounded-full px-4 py-1 flex items-center gap-2 hover:bg-gray-50">
            {name}
            <button type="button" onClick={handleTag}><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth="2" stroke="currentColor" className="w-4 h-4"><path strokeLinecap="round" strokeLinejoin="round" d="M6 18L18 6M6 6l12 12" /></svg></button>
        </div>
    )
}
...
const HomePage = () => {
    ...
    return (
        <AppContainer>
            <div className="grid gap-4 grid-cols-1 lg:grid-cols-[28rem_1fr] place-items-start py-4 text-base">
